...
<script>
import { getIndicatorById, updateIndicator, getIndicatorCategories } from '@/api/indicator';
...
export default {
  ...
  methods: {
    ...
    async loadIndicator() {
      this.loading = true;
      try {
        const response = await getIndicatorById(this.indicatorId);
        if (response.code === 200) {
    ...
    handleSubmit() {
      this.$refs.indicatorForm.validate(async (valid) => {
        ...
        try {
          const response = await updateIndicator(this.indicatorId, this.indicatorForm);
          if (response.code === 200) {
